...
Also, malloc leave garbage in memory because of performance -- check your code to make sure that a program all program values are initialized.
...
\begin{enumerate}
\item Best fit may take less time than a full heap scan. When a block of perfect size or of perfect size within a threshold is found, that can be returned, depending on what edge-case policy you have.
\item Worst fit follows this as well. Your heap could be represented with the max-heap data structure and each allocation call could simply pop the top off, reheapify, and possibly insert a split memory block.
