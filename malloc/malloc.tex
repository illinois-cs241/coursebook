...
The SLUB allocator is a slab allocator that serves different needs for the Linux kernel \todo{citation needed}.
Imagine you are creating an allocator for the kernel, what are your requirements?
Here is an imaginary shortlist.
...
\item Then, you want the actual memroy to be as contiguous as possible to make use of caching. Every time a system call is performed, the kernel's pages need to get loaded into memory.
    This means that if they are all contiguous, the processor will be able to cache them easier.
\item Lastly, you want your allocations to be fast. This isn't the most paramount because we can imagine that our kernel on boot-up does a lot of allocations and does comparatively few during the execution.
\end{enumerate}
