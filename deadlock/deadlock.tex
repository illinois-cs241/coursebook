...
\begin{figure}[H]
	\centering
	\includegraphics[width=.6\textwidth,alt={Resource allocation graph with three processes and two resources. Resource 1 is held by process 1 and requested by processes 2 and 3; resource 2 is held by process 2 and requested by process 3. The graph has no cycle.}]{deadlock/drawings/rag.eps}
...
For example, if process 1 holds resource A, process 2 holds resource B and process 1 is waiting for B and process 2 is waiting for A, then processes 1 and 2 will be deadlocked (see Figure~\ref{deadlockfigure}).
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.5\textwidth,alt={The resource allocation graph with one more edge: process 1 now also requests resource 2. Dashed edges mark the resulting cycle between processes 1 and 2 and the two resources.}]{deadlock/drawings/deadlock.eps}
	\caption{Graph based Deadlock}
	\label{deadlockfigure}
\end{figure}

In this graph, process 1 holds resource 1 and is requesting resource 2, while process 2 holds resource 2 and is requesting resource 1.
The dashed edges mark that cycle, so processes 1 and 2 are deadlocked, and process 3, which is requesting both resources, is stuck behind them.
...
There are four conditions for deadlock, and they are \emph{necessary}: if a system is deadlocked, all four hold, so a system that breaks any one of them cannot deadlock.
In general they are not \emph{sufficient}: all four can hold and the system can still make progress, for example when a resource has several interchangeable instances and a process outside the cycle returns one.
If every resource has a single instance, the conditions are sufficient as well, so a circular wait is a deadlock.
...
\begin{proof} Suppose every resource has a single instance and a process waits for at most one resource at a time.
	Call a set of processes $D$ \emph{deadlocked} if every process in $D$ is waiting for a resource held by another process in $D$.
	When $D$ is every process, this is the whole-system deadlock from the start of the chapter.
	We show two things: if a set of processes is deadlocked, all four Coffman conditions hold; and if the system has mutual exclusion, hold and wait, and no pre-emption, then any cycle in the resource allocation graph is a deadlock.

	$\rightarrow$ Suppose $D$ is deadlocked.

	\begin{itemize}
		\item Mutual exclusion: a process in $D$ is waiting for a resource that someone else holds. If that resource could be shared, it would simply be given to the waiting process, which would then not be waiting.
		\item Hold and wait: take any $p \in D$ waiting for resource $r$. The holder $q$ of $r$ is also in $D$, so $q$ is itself waiting -- while holding $r$.
		\item No pre-emption: if the system could take $r$ away from $q$, it could give $r$ to $p$, and $p$ would not be stuck.
		\item Circular wait: start at any $p_0 \in D$. Let $p_1$ be the holder of the resource $p_0$ waits for, $p_2$ the holder of the resource $p_1$ waits for, and so on. Every $p_i$ is in $D$, so this chain never stops. But $D$ is finite, so some process eventually repeats: $p_i = p_j$ for some $i < j$. Then $p_i \rightarrow p_{i+1} \rightarrow \cdots \rightarrow p_{j-1} \rightarrow p_i$ is a cycle in the resource allocation graph.
	\end{itemize}

	$\leftarrow$ Suppose the system has mutual exclusion, hold and wait, and no pre-emption, and the resource allocation graph contains a cycle $p_1 \rightarrow r_1 \rightarrow p_2 \rightarrow r_2 \rightarrow \cdots \rightarrow p_k \rightarrow r_k \rightarrow p_1$.
	That is, $p_i$ is waiting for $r_i$, which is held by $p_{i+1}$, and $p_{k+1}$ means $p_1$.
	For contradiction, suppose some process on the cycle eventually gets the resource it is waiting for, and let $p_i$ be the \emph{first} one to do so, at time $t$.
	Resource $r_i$ has a single instance and cannot be shared, so $p_{i+1}$ must have given it up before time $t$.
	There is no pre-emption, so nobody took $r_i$ away from $p_{i+1}$; $p_{i+1}$ released it itself.
	Because of hold and wait, $p_{i+1}$ does not release anything while it is still waiting, so $p_{i+1}$ must have been granted $r_{i+1}$ before time $t$.
	That contradicts $p_i$ being the first.
	So no process on the cycle ever gets its resource: the processes on the cycle are deadlocked.
\end{proof}

The only place the proof used single instances is the $\leftarrow$ direction: if $r_i$ had a second copy, a process outside the cycle could release that copy and let $p_i$ continue.
That is why, with multi-instance resources, a cycle is necessary for deadlock but not sufficient.
...
Though deadlock is avoided, consider a variant of this solution where the two students reach for the items in opposite orders: one reaches for the pen first and the other reaches for the paper first.
Each grabs their first item, fails to get the second, puts the first down, and tries again.
If they keep doing this in lockstep, both are busy but no work will be done.
...
You must prove that a particular system is free of livelock, usually with what is known as an invariant.
...
The ostrich algorithm accepts that a malicious or badly written program can still deadlock; the operating system simply doesn't try to prevent it.
...
Dijkstra's original version of the problem used forks and a bowl of spaghetti, and a skeptic might point out that you can eat spaghetti with one fork, so many retellings use chopsticks instead -- nobody eats with one chopstick.
The code and figures in this chapter still say fork; the two words mean the same thing.
Each chopstick is shared with a neighbor.

\begin{figure}[H]
	\centering
	\includegraphics[width=.5\textwidth,alt={Six philosophers, drawn as plates, sit around a round table with one fork between each neighbouring pair. The philosophers are the processes and the forks are the resources.}]{deadlock/drawings/dining.eps}
...
This looks good, but what if everyone picks up their left fork and then waits for their right fork?
We have deadlocked the program.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.9\textwidth,alt={Three stages at the table: first idle, then every philosopher picks up their left fork, then every philosopher waits for the right fork that a neighbour already holds. The waits form a cycle, marked as deadlock.}]{deadlock/drawings/dining_naive.eps}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.9\textwidth,alt={A time sequence in which every philosopher repeatedly picks up their left fork, fails to get the right one, puts the left fork down, and tries again. The pattern repeats forever and nobody eats.}]{deadlock/drawings/dining_livelock.eps}
...
\begin{proof} The arbitrator solution doesn't deadlock
	...
	The proof is about as simple as it gets.
	A philosopher only picks up forks while holding the arbitrator, and puts both forks down before letting go of it.
	So whoever holds the arbitrator finds every fork on the table and never waits for one.
	Everyone else is waiting only for the arbitrator, which is held by a philosopher who is not waiting.
	A cycle in the resource allocation graph needs every philosopher on it to be waiting, so no cycle can form, and without circular wait there is no deadlock, which is what we needed to show.
	
\end{proof}

\begin{figure}[H]
	\centering
	\includegraphics[width=.9\textwidth,alt={Every philosopher asks a single central lock, the arbitrator, for permission. Only the philosopher holding the lock picks up both forks, so no cycle of waiting can form.}]{deadlock/drawings/dining_arbitrator.eps}
...
\begin{proof} Stallings' Solution Doesn't Deadlock.
	...
	A philosopher $p_i$ needs resources $r_i$ and $r_{(i+1) \bmod n}$.
	...
	Now resources $r_i$ and $r_{(i+1) \bmod n}$ each have only one philosopher that could use them.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.9\textwidth,alt={The table with one seat crossed out, leaving five philosophers and six forks. As the philosophers pick up forks and reach for their neighbours, the extra fork lets at least one of them eat, so the system avoids deadlock.}]{deadlock/drawings/dining_stalling.eps}
	\caption{Stallings' solution almost deadlock}
\end{figure}

Because one seat is empty, there are more forks than philosophers.
Even if all five remaining philosophers hold their left fork, one fork is still free, so at least one philosopher can pick up a second fork and eat.
...
\begin{proof} Dijkstra's Solution Doesn't Deadlock
	...
	Let's number the philosophers $\{p_0, p_1, \ldots, p_{n-1}\}$ and the forks $\{r_0, r_1, \ldots, r_{n-1}\}$, where philosopher $p_i$ needs forks $r_i$ and $r_{(i+1) \bmod n}$.
	Everyone picks up their lower-numbered fork first.
	For $p_0$ through $p_{n-2}$ that means $r_i$ and then $r_{i+1}$.
	The last philosopher, $p_{n-1}$, needs $r_{n-1}$ and $r_0$, so they pick up $r_0$ first and then $r_{n-1}$ -- the reverse of everyone else.
	Notice that $p_0$ and $p_{n-1}$ both want $r_0$ as their \emph{first} fork, and nobody picks up their second fork before their first.
	So whichever of the two does not hold $r_0$ is holding no fork at all.
	A philosopher holding nothing cannot be part of a cycle in the resource allocation graph, because everyone on a cycle holds the fork that the previous philosopher is waiting for.
	There are two cases.

	If $p_{n-1}$ does not hold $r_0$, then $p_{n-1}$ holds nothing, and we can take $p_{n-1}$ out of the picture.

	If $p_{n-1}$ does hold $r_0$, then $p_0$ holds nothing, and we can take $p_0$ out of the picture.

	Either way, at most $n-1$ philosophers are sharing the $n$ forks, which is exactly the situation in Stallings' proof above, so circular wait cannot happen.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.9\textwidth,alt={The same stages under Dijkstra ordering: every philosopher reaches for their lower-numbered fork first, so one philosopher reaches the opposite way from the rest. That breaks the circular wait, and the system does not deadlock.}]{deadlock/drawings/dining_partial.eps}
