...
\begin{figure}[H]
	\centering
	\includegraphics[width=.5\textwidth,alt={A two-column page table: each row maps a page number, counting 0, 1, 2 and so on, to the number of the physical frame that holds that page. Consecutive pages can map to frames anywhere in physical memory.}]{ipc/drawings/frame_table.eps}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.5\textwidth,alt={The same 32-bit address 11110000111100001111000010101010 split into three fields: a 10-bit index into the top-level table, a 10-bit index into a second-level table, and a 12-bit offset within the frame.}]{ipc/drawings/three_address_split.eps}
