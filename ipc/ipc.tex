...
\begin{figure}[H]
	\centering
	\includegraphics[width=.5\textwidth,alt={A two-column page table with columns Page Number and Frame Number: page 0 maps to frame 42, page 1 to frame 45, page 2 to frame 30, and the table continues. Consecutive pages can map to frames anywhere in physical memory.}]{ipc/drawings/frame_table.eps}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.5\textwidth,alt={The 32-bit virtual address 11110000111100001111000010101010 split into a 20-bit page number, 11110000111100001111, and a 12-bit offset, 000010101010.}]{ipc/drawings/address_split.eps}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.0\textwidth,alt={Single-level translation: the top 20 bits of the address index the page table to find a frame number, and the low 12 bits are the offset of the byte within that physical frame.}]{ipc/drawings/level_split.eps}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.0\textwidth,alt={The example address translated: its page number selects the page table entry holding frame 357, and its offset selects the byte inside physical frame 357.}]{ipc/drawings/level_split_filled.eps}
	\caption{One level dereference example}
\end{figure}

In this example, the top 20 bits 11110000111100001111 select the page table entry that holds frame 357.
The offset 000010101010 then selects the byte inside frame 357.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.5\textwidth,alt={The same 32-bit address 11110000111100001111000010101010 split into three fields: Index 1, the top 10 bits 1111000011, which index the top-level table; Index 2, the next 10 bits 1100001111, which index a second-level table; and Offset, the low 12 bits 000010101010, which locate the byte within the frame.}]{ipc/drawings/three_address_split.eps}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.0\textwidth,alt={Two-level translation: the top 10 bits index the base directory to find a sub-table, the next 10 bits index that sub-table to find a frame number, and the last 12 bits locate the byte within the physical frame.}]{ipc/drawings/multi_level_split.eps}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.0\textwidth,alt={The example address translated through two levels: directory entry 173 leads to sub-table 173, whose entry gives frame 241, and the offset selects the byte inside frame 241.}]{ipc/drawings/multi_level_filled.eps}
	\caption{Full page example dereference}
\end{figure}

Here the top 10 bits 1111000011 select directory entry 173, which points to sub-table 173.
The middle 10 bits 1100001111 select the entry holding frame 241, and the offset 000010101010 selects the byte inside frame 241.
	 
\subsubsection{Calculating Size Concerns}
	 
Now some calculations on size.
The top 10 bits of the address are an index \emph{into} the directory, but each directory entry has to say where its sub-table lives in physical memory, which takes a frame number.
So, like the sub-table entries, each directory entry is 4 bytes.
With $2^{10}$ entries of 4 bytes each, the whole top-level directory is 4KiB -- exactly one frame.
...
Thus, the total memory overhead for our multi-level page table has shrunk from 4MiB for the single-level implementation to just three tables: the 4KiB top-level directory plus two 4KiB sub-tables, $3 \times 4\text{KiB} = 12$KiB in total.
Here's why.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth,alt={Five processes in a pipeline: ls -1, cut, sort, uniq, and tee dirents. Arrows connect each process standard output, file descriptor 1, to the next process standard input, file descriptor 0.}]{ipc/drawings/pipe_process.eps}
...
If a process writes to a pipe that still has a reader, the write succeeds as long as the pipe's buffer has room.
If the pipe is full, the write blocks until a reader makes room; on a non-blocking pipe it instead writes what fits and returns early, or fails with \keyword{EAGAIN} if nothing fits.
...
Sometimes a program needs to know how big a file is, for example to allocate a buffer big enough to hold all of it.
For files whose size fits in a \keyword{long} (see below), \keyword{fseek} and \keyword{ftell} are a simple way to find out.
...
All future reads and writes on this stream will honor this position.
...
The \keyword{fseek}/\keyword{ftell} trick has a catch: \keyword{ftell} returns a \keyword{long}, and C only requires a \keyword{long} to be \textbf{4 bytes big}.
Where that is the case -- 32-bit Linux, and even 64-bit Windows -- the largest position \keyword{ftell} can report is $2^{31}-1$ bytes, a little under 2 gibibytes.
