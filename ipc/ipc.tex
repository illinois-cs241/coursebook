...
\begin{figure}[H]
  \centering
  \includegraphics[width=.5\textwidth]{ipc/drawings/frame_table.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.5\textwidth]{ipc/drawings/address_split.png}
  \caption{Splitting Address}
\end{figure}

We first go to the page table and find the frame number -- with pointer arithmetic \keyword{page\_table\_entry *ptr + num}
...
The process will believe that the address look continuous, but the top 20 bits are used to figure out \keyword{page\_num},
...
First, you take the top level page table and find the \keyword{Index1}'th entry.
...
In one diagram, it looks like the following image.

\begin{figure}[H]
  \centering
  \includegraphics[width=.6\textwidth]{ipc/drawings/multi_address_split.png}
...
How your friendly neighborhood TAs remember it is you have to \textit{read before you can write, or reading comes before writing}.
...
\end{lstlisting}
Which grabs 20 lines of input from urandom. \keyword{head} will terminate after 20 newline characters have been read. Well what about \keyword{cat}?
...
A lot of time, you'll want to use \keyword{pipe2} in combination with dup. Take for example the simple program in the command line.

\begin{lstlisting}[language=bash]
$ ls -1 | cut -f1 -d.
\end{lstlisting}

This command take the output of \keyword{ls -1} which lists the content of the current directory on one line each and pipes it to cut.
...
The basic UNIX motto is that anything that you can \keyword{fseek} or move to an arbitrary position in, you should be able to \keyword{fdopen}. Files satisfy this behavior, shared memory also, terminals, etc.
...
A named pipe \keyword{mkfifo} is simply a pipe that you call \keyword{open(2)} on with read and/or write permissions.
...
\textbf{Danger} With portability you lose something important, the ability to tell an error. Take for example the advice that we gave you above. You can \keyword{fopen} a file descriptor and get a \keyword{FILE*} object but \textbf{it won't be the same as a file} meaning that certain calls will fail or act weirdly.
...
Other processes can continue to use their own file-handle. Remember, everything is copied over when a child is created, even the relative positions of the files. As you might have observed with using \keyword{fork}, there is a quirk of the implementation of files and their caches on Ubuntu that will rewind a file descriptor once a file has closed. As such, make sure to close before forking or at least don't trigger a cache inconsistency which is much harder.
