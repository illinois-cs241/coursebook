...
A typical block size on Linux is 4KiB or $2^{12}$ addresses, though you can find examples of larger blocks.
...
\item \keyword{fread} reads a certain number of bytes from a file. An error is returned if already at the end of the file when which you must call \keyword{feof()} to check if the program attempted to read \textit{past} the end of the file.
...
Take the advice that we gave you above.
You can \keyword{fopen} a file descriptor and get a \keyword{FILE*} object but \textbf{it won't be the same as a file} meaning that certain calls will fail or act weirdly.
