...
\item $E[R_i] = \sum\limits_{j = 0}^i E[N_j] * E[S_j]$ Which simply says that you need to wait for all jobs with a higher priority and the same to go before you can go.
  ...
  Meaning that we are tied to wait times and service times of all other processes. Again, if we keep average wait times low or we can assume that the jobs don't take that much time to service on average, this becomes better than FCFS.
  Another way of thinking about it is adding in another term $p_i$

  \[
      E[R] = E[S] \sum\limits_{i=0}^k p_i \sum\limits_{j = 0}^i \lambda_j E[W_j] / p_j

  \]

  Our final inequality will be taking out the $\lambda$ from the sum for comparison

  \[
      E[R] \leq \lambda * E[S] \sum\limits_{i=0}^k p_i \sum\limits_{j = 0}^i E[W_j] / p_j
  \]

  So after all of this alphabet soup We finally have some expression for when we can compare this to the other distributions.
  Here is FCFS
  ...
  If we want a better response time that FCFS, we can keep the expected wait times down or we can again expect a higher number of low priority processes so that on average our weight times are lower.
