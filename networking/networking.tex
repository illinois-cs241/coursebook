...
To get your IP Address from the command line use \keyword{ifconfig} (or Windows' \keyword{ipconfig}) However this command generates a lot of output for each interface, so we can filter the output using grep
...
Why is network order defined to be big endian? The simple answer is that \link{https://tools.ietf.org/html/rfc1700} RFC1700 says so.
...
There are four parts the method e.g.~GET,POST,\ldots{}); the resource (e.g. / /index.html /image.png); the protocol ``HTTP/1.0'' and two new lines (\texttt{\\r\\n\\r\\n})
...
\begin{enumerate}
  ...
  One can use the macros \keyword{NI\_MAXHOST} to denote the maximum length of a hostname, and \keywrod{NI\_MAXSERV} to denote the maximum length of  a port. \keyword{NI\_NUMERICHOST} gets the hostname as a numeric IP address and similarly for \keyword{NI\_NUMERICSERV} though the port is usually numeric to begin with. The \href{https://man.openbsd.org/getnameinfo.3#NI\_NUMERICHOST}{Open BSD man pages have more}
