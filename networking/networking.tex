...
However, very little web traffic is IPv6 based on comparison as of 2018 \cite{internet_society_2018}
...
Also to specifying IPv4 or IPv6, one can specify either with \keyword{AF\_UNSPEC}.
...
Sockets are created with a domain \keyword{AF\_INET} for \keyword{IPv4} or \keyword{AF\_INET6} for \keyword{IPv6}, \keyword{socket\_type} is whether to use UDP, TCP, or other some other socket type, the \keyword{protocol} is an optional choice of protocol configuration for our examples this we can just leave this as 0 for default.
...
int client_id = accept(passive_socket,
 ...
 getnameinfo((struct sockaddr *) &clientaddr,
  ...
  If you shutdown the socket for further writing at the server end, then a moment later, a blocked \keyword{read} call could return 0 to indicate that no more bytes are expected.
  ...
  If you shutdown a socket for further read, all processes are affected because you've changed the socket, not just the file descriptor.
...
Note that we pass \keyword{SOCK\_DGRAM} instead of stream.
...
To marshal a linked list, it is unnecessary to send the link pointers, just stream the values.
