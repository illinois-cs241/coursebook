...
TCP creates a \emph{pipe} between two machines and abstracts away the low-level packet-nature of the Internet. Thus, under most conditions, bytes sent over a TCP connection deliverd and uncorrupted.
...
\item Flow Control.
  ...
  There are also other problem of making sure the local network's traffic is stable.
...
\begin{enumerate}
  ...
  By default, a port is released after a period of time when the server socket is closed.
...
When you're reading data from a disk, that delay is short, but when you're reading from a slow network connection, requests long time.
