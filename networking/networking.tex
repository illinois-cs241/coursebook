...
The Open Systems Interconnection 7 layer model (OSI Model) is a sequence of segments that define standards for both infrastructure and protocols for forms of radio communication, in our case the Internet.
...
\begin{figure}[H]
  \centering
\includegraphics[width=.8\textwidth,alt={The IPv6 header drawn as 32-bit rows: version, traffic class and flow label; then payload length, next header and hop limit; then large blocks for the source and destination addresses.}]{networking/drawings/ipv6_datagram.eps}
\caption{IPv6 Datagram divisibility}
\end{figure}

In the header above, the first 32 bits hold a 4-bit version, an 8-bit traffic class, and a 20-bit flow label.
The next 32 bits hold the 16-bit payload length, the 8-bit next header field, and the 8-bit hop limit, followed by the 128-bit source and destination addresses.
...
Passing \keyword{AF\_UNSPEC} (unspecified) accepts either IPv4 or IPv6.
...
\begin{figure}[H]
  \centering
\includegraphics[width=.8\textwidth,alt={The TCP header drawn as 32-bit rows: source and destination ports; sequence number; acknowledgment number; data offset, reserved bits, flags and window; checksum and urgent pointer; then variable-length options and padding.}]{networking/drawings/tcp_header.eps}
\caption{Extra: TCP Header Specification}
\end{figure}

In the header above, each row is 32 bits wide.
The 16-bit source and destination ports come first, then the 32-bit sequence and acknowledgment numbers, then the data offset, reserved bits, flags, and a 16-bit window size, then the checksum and urgent pointer, and finally any options, padded to a multiple of 32 bits.
...
The other modes for `family` are \keyword{AF\_INET} and \keyword{AF\_UNSPEC} which mean IPv4 and unspecified respectively.
...
\begin{enumerate}
  \item "GET \%s HTTP/1.1" This is the request method interpolated with the path. This means to perform the GET method on the path using the HTTP/1.1 protocol version.
  \item "Host: \%s" The name of the server we want to talk to.
    HTTP/1.1 requires this header because one IP address often hosts many different websites, and the server needs to know which one we meant.
  \item "Connection: close" Means that as soon as the response is over, please close the connection.
    HTTP/1.1 keeps connections open by default so that more requests can reuse them.
    Asking the server to close keeps our client simple: the end of the response is just the end of the file, so we don't have to parse \keyword{Content-Length}.
...
\item The protocol ``HTTP/1.1''
\item A new line (\keyword{\textbackslash r\textbackslash n}). Requests always have a carriage return.
\item Any other knobs or switch parameters, called headers, one per line. HTTP/1.1 requires at least the \keyword{Host} header.
...
\keyword{GET /nosuchfile.html HTTP/1.1} Then the first line includes the well-known \keyword{404} response code.
...
HTTP/1.1 is not the end of the story.
Without \keyword{Connection: close}, an HTTP/1.1 connection stays open after the response (a persistent connection), so a browser can send many requests without paying for a new TCP handshake each time.
HTTP/2 keeps the same methods, headers, and status codes, but sends them as binary frames and multiplexes many requests at once over a single TCP connection \cite{rfc9113}.
HTTP/3 goes further and runs over QUIC, which is built on UDP, so that one lost packet doesn't stall every request behind it the way it does on a single TCP stream (TCP head-of-line blocking) \cite{rfc9114}.
Your browser speaks all of these, but the text protocol above is still what you'll see when you talk to a server by hand.
...
\begin{enumerate}
  ...
  Passive server sockets do not send or receive data themselves.
  Instead, they wait for incoming connections.
  Additionally, a passive server socket is not used to talk to any particular client, so it remains open when a peer disconnects.
  ...
  It is possible, though uncommon, to call bind on a TCP client; normally the operating system picks the client's port automatically.
  ...
  By default, a port is not released immediately when the server socket is closed.
  Instead, the port enters a ``TIME-WAIT'' state.
  ...
  To be able to immediately reuse a port, specify \keyword{SO\_REUSEADDR} before binding to the port.
...
bind(...);
  ...
  Here's \href{http://stackoverflow.com/questions/14388706/socket-options-so-reuseaddr-and-so-reuseport-how-do-they-differ-do-they-mean-t}{an extended stackoverflow introductory discussion of \keyword{SO\_REUSEADDR} and \keyword{SO\_REUSEPORT}}.
...
A system called ``DNS'' (Domain Name System) is used.
...
There are good reasons to prefer epoll over select, because select's interface has fundamental problems.
It can only watch descriptor numbers below \keyword{FD\_SETSIZE} (usually 1024), and every call costs time proportional to the number of descriptors watched.
...
\item ASCII, Unicode Transformation Format 8 (UTF-8), some other encoding?
