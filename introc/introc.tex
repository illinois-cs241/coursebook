...
Another problem is that they can't be nested infinitely -- there is an bounded depth where they need to stop.
...
\end{lstlisting}
        ...
        Note that an else is matched with the most recent else. A subtle bug related to a mismatched if and else statement, is the \herf{dangling else problem}{https://en.wikipedia.org/wiki/Dangling_else}.
...
If you want a fixed width integer type, for more portable code, you may use the types defined in stdint.h, and , which are of the form [u]int\emph{width}_t, where u (which is optional) represents the signedness, and width is any of 8, 16, 32, and 64.
...
\begin{enumerate}
	...
	\item \keyword{\&a} is the address-of operator. This takes the an element and returns its address.
