...
\end{lstlisting}
	    ...
	    \item \keyword{extern} declares a variable without defining it. It tells the compiler that the variable exists and is defined in another source file or library, so the file compiles, and the linker connects the name to its definition later.
...
\begin{itemize}
	...
	\item \keyword{++} is the increment operator. You can use it as a prefix or postfix, meaning that the variable that is being incremented can either be before or after the operator. \keyword{a = 0; ++a == 1} and \keyword{a = 0; a++ == 0} (in both cases, \keyword{a} is 1 afterwards).
