...
\end{lstlisting}
        ...
        That is typically why programmers either put the full declaration in a header file or abstract the creation and the interaction away so that users cannot access the internals of our struct.
...
\item \keyword{char} Represents exactly one byte of data.
  ...
  \keyword{unsigned char} and \keyword{signed char} mean the exact same thing.
