...
\keyword{printf} will use the buffer provided by the \keyword{FILE*} abstraction to store bytes until ready to write.
One can force it to write by sending a newline, or by calling fflush() on the stream, which is stdout in this case.
