...
To print data into a C string, use \keyword{sprintf}, or better \keyword{snprintf}, or best \keyword{asprintf}.
\keyword{sprintf} does no bounds checking, so only use it when the output is guaranteed to fit in the buffer -- think about printing a 32-bit integer, it will never be more than 12 bytes including the sign and the NUL byte.
\keyword{snprintf} is told the size of the buffer and never writes past it.
It returns the number of characters that would have been written, excluding the terminating byte, so a return value greater than or equal to the buffer size means the output was truncated.
\keyword{asprintf} is best: it allocates a buffer on the heap that is large enough for the entire result, so the output can never be truncated or overflow, at the cost of a heap allocation that the caller must later \keyword{free}.
