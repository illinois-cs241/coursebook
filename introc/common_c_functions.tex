...
\keyword{printf} includes an internal buffer so, to increase performance \keyword{printf} may not call \keyword{write} every time \keyword{printf} is called.
Another quirk of printf is calling \keyword{write} on a newline character.
If one wants \keyword{printf} to call write without a newline, they can use \keyword{fflush(\ FILE*\ inp\ )}.
...
printf can also be used with file descriptors as well.
...
Note that, unlike \keyword{gets}, \keyword{fgets} copies the newline into the buffer, which may be discarded.
...
\end{lstlisting}
        ...
        This often involves and errno trampoline as shown below.
