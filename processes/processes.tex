...
Importantly, the kernel creates the first process \keyword{init.d} (an alternative is system.d). \emph{init.d} boots up programs such as graphical user interfaces, terminals etc.
What is important is the operating system only explicitly creates one process by default. All other processes are instantiated by using the system calls \keyword{fork} and \keyword{exec} from that single process.
...
Common errors include \keyword{EAGAIN} and \keyword{ENOMEM} Which are essentially "try again", and "out of memory".
...
If the parent process wants to wait for the child to finish, \keyword{waitpid} (or \keyword{wait}).
...
However, none of the UNIX programs are designed to follow this policy, for the sake of simplicity.
...
\begin{enumerate}
    \item File descriptors are preserved after an exec. That means if you open a file, and you forget to close it, it remains open in the child.
          ...
          The one exception to this is if the file descriptor has the Close-On-Exec flag set (O_CLOXEC) -- we will go over setting flags later.
...
Passing something along the lines of argv[1] = "; sudo su" is a huge security risk called \href{https://en.wikipedia.org/wiki/Privilege_escalation}{privilege escalation}.
