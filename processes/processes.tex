...
This is a standard (or many standards now) that an operating system must implement to be POSIX compatable -- most systems that we'll be studying are almost POSIX compatable due more to political reasons.

Before we talk about POSIX systems, we should understand what the idea of a kernel is generally.
In an operating system there are two spaces, kernel space and user space.
...
This is where there is a central clock in a standard laptop or desktop.
There are other operating systems that relax the central clock requirement (distributed) or the ``standardness'' of the hardware (embedded systems).
...
A zine from Julia Evans that gives some details \cite{evans_2018}.
