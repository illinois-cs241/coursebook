...
Note, this limit is only for the user, which means if you fork bomb, then you won't be able to kill all created process since calling \keyword{killall} requires your shell to \keyword{fork()} \ldots{}
Quite unfortunate.
...
\begin{itemize}
    \item The child process does not start from main. Instead, it executes the next line after the \keyword{fork()} as the parent process does.
