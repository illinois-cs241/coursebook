...
The code does not stop two threads from reading and writing \keyword{sum} at the same time.
...
W can release a waiting thread that can now make all of the calls that we were not allowed to call inside the signal handler itself e.g. \keyword{printf}.
...
If we added 20 integers (1,2,3\ldots{}) to the queue and did not dequeue any items then values, \keyword{17,18,19,20} would overwrite the \keyword{1,2,3,4}.
...
With more than two billion enqueue operations, the int value of \keyword{in} will overflow and become negative!
...
You will find out what this call to \keyword{mmap} does later -- just assume for the time being that it creates memory that is shared between processes.
