...
If three threads called \keyword{push()} at the same time, the lock \keyword{m1} ensures that only one thread at time manipulates the stack on push or is_empty -- Two threads will need to wait until the first thread completes
...
Remember that the psuedo-code outlined below is part of a larger program.
...
static void remove_from_list(cond_t *cv, cv_node *ptr) {
...
void cond_signal(cond_t *cv) {
  for (cv_node *iter = cv->head; iter; iter = iter->next) {
    ...
    // is woken up
    if (iter->is_awoken == 0) {
      // DONT remove from the linked list here
...
The signal and broadcast functions merely tell either one thread or all threads respectively that they should be worken up.
