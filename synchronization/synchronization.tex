...
C11 provides two versions of compare-and-swap.
The \emph{strong} version fails only if the value really differed from what was expected.
The \emph{weak} version may also fail \textbf{spuriously}, even when the value matched, which lets it compile to faster instructions on some CPUs, such as ARM.
These are similar to the spurious wakeups that you'll see in condition variables below.
We retry in a loop anyway, so a spurious failure only costs one more iteration, and we use the faster weak version.
...
A semaphore has no owner, so any thread can call \keyword{sem\_post}, even one that never waited.
In the following code, thread 2's stray \keyword{sem\_post} lets threads 1 and 3 into the critical section at the same time.
...
With a mutex, thread 2's unlock is an error, because thread 2 does not own the mutex.
An error-checking mutex returns \keyword{EPERM}; with a default mutex the behavior is undefined, so don't rely on it.
...
Also, binary semaphores are different than mutexes because a mutex must be unlocked by the thread that locked it, whereas any thread can call \keyword{sem\_post} on a semaphore, even one it never waited on.
...
Remember \keyword{sem\_wait} will wait if the semaphore's count has been decremented to zero (by other threads calling \keyword{sem\_wait}).
...
Sketch \#1 is only half of the story.
With \keyword{sitems} starting at 0 and \keyword{sremain} starting at 10, \keyword{pop} blocks while the stack is empty and \keyword{push} blocks once 10 items are on it, which is what we want.
But nobody ever calls \keyword{sem\_post}: \keyword{sitems} never goes up, so every \keyword{pop} blocks forever, and \keyword{sremain} never comes back up, so after 10 pushes every \keyword{push} blocks forever too.
Each successful \keyword{push} creates an item, so it must post \keyword{sitems}; each successful \keyword{pop} frees a space, so it must post \keyword{sremain}.
The question is \emph{where} to put those posts.
...
Notice that \keyword{sem\_post} calls \keyword{pthread\_cond\_signal} every single time.
...
\begin{figure}
\centering
\includegraphics[width=.5\textwidth,alt={A ring of 16 slots numbered 0 to 15, with items C, D, E and F in slots 2 to 5. The out index points at the oldest item and the in index at the next free slot; both move clockwise and wrap from slot 15 back to slot 0.}]{synchronization/drawings/ring_buffer.eps}
