...
The \keyword{\&\&} and \keyword{||} operators execute commands sequentially. \keyword{\&\&} only executes the next command if the previous command succeeds, and \keyword{||} only executes the next command if the previous command fails.
...
\begin{figure}[H]
\centering
\includegraphics[width=.7\textwidth,alt={Six 4-byte boxes in a row: one box for height, two for the data pointer, one for width, and two for the encoding pointer, with no gaps between the fields.}]{appendix/drawings/struct_clean.eps}
...
\begin{figure}[H]
\centering
\includegraphics[width=.7\textwidth,alt={The same struct drawn as eight 4-byte boxes: height, a padding box marked with a question mark, two boxes for data, width, a second padding box, then two boxes for encoding. The padding after each int keeps the following 8-byte pointer aligned.}]{appendix/drawings/struct_slop.eps}
...
Also, every message has to be serialized by the sender and deserialized by the receiver -- or, at the least, copied into and out of a mailbox.
...
Is a pure-software mutex such as Peterson's algorithm ever used in real systems?
Yes!
With a bit of searching, it is possible to find Peterson's algorithm in production on specific simple mobile processors today.
...
\section{Scheduling Conceptually}\label{sec:scheduling_conceptually}
...
Let $T$ be the maximum amount of time that a process runs; every process finishes in a finite amount of time.
...
\item Queueing theory involves a random variable controlling the interarrival time -- or the time between two different processes arriving.
  We won't name this random variable, but we will assume that processes arrive as a Poisson process with rate $\lambda$, meaning $\lambda$ processes arrive per unit of time on average.
  This means the interarrival time is exponentially distributed with mean $\frac{1}{\lambda}$: the probability density of the next process arriving $t$ units after the previous one is $\lambda e^{-\lambda t}$.
...
We will denote two variables that $\frac{1}{\mu}$ is the mean of the service time (so $\mu$ is the service rate) and that the coefficient of variation $C$ is defined as $C^2 = \frac{var(S)}{E[S]^2}$ to help us control for processes that take a while to finish.
...
As before, $T$ is the maximum amount of time a process can run for, since our processes cannot run infinitely long.
...
\begin{figure}[H]
  \centering
\includegraphics[width=.8\textwidth,alt={The IPv4 header drawn as 32-bit rows. Row one holds the version, header length, service type and total length; row two the identification, flags and fragment offset; row three the TTL, protocol and header checksum. Then come full-width rows for the source and destination addresses and a final row of variable-length options plus padding.}]{appendix/drawings/ip_datagram.eps}
...
\item Internal Routing Protocols.
  ...
  Common examples are OSPF and IS-IS, which are link-state protocols: every router learns the whole map of the ISP's network and calculates the shortest path to each destination.
\item External Routing Protocols.
  ...
  The external routing protocol of today's Internet is BGP, the Border Gateway Protocol.
  Instead of computing shortest paths, BGP routers advertise which networks they can reach, and this is how information about the "foreign" networks is disseminated to each ISP.
  Each ISP then picks among the advertised routes according to its own policies.
\end{enumerate}
...
\bibliography{appendix/appendix}
