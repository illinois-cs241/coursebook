...
From the Linux man page of sigprocmask, here are the how parameters

\begin{itemize}
\item \keyword{SIG\_BLOCK}. The set of blocked signals is the union of the current set and the
  set argument.
\item \keyword{SIG\_UNBLOCK}. The signals in set are removed from the current set of blocked
  signals. It is permissible to attempt to unblock a signal which is not blocked.
\item \keyword{SIG\_SETMASK}. The set of blocked signals is set to the argument set.
\end{itemize}
