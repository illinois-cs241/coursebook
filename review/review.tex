...
\item How would you print the values of the variables \keyword{a}, \keyword{mesg}, \keyword{val} and \keyword{ptr} to a string? Print a as an integer, mesg as a C string, val as a double and ptr as a hexadecimal pointer. You may assume the mesg points to a short C string (\textless{}50 characters). Bonus: How would you make this code robust, so that it copes with any length of \keyword{mesg} and with \keyword{malloc} returning \keyword{NULL}? Hint: printing a large \keyword{val} with \keyword{\%f} can take over 300 characters.
...
Use a mutex lock and condition variables to implement a reader-writer solution.
An incomplete attempt is shown below. Though this attempt is thread safe (thus sufficient for demo day!), it does not allow multiple threads to calculate \keyword{shortest} path at the same time and will not have sufficient throughput.
