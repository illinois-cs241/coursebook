...
The Computer Fraud and Abuse Act (CFAA) is a broad, and arguably terrible law, that casts non-authorized use of a `protected computer' as a crime.
...
\item Execute the plan with caution.
  If at any point something seems wrong, stop and weigh the risks again before continuing.
\end{enumerate}
...
\item Address Space Layout Randomization (ASLR).
  ...
  Without ASLR, an attacker could, for example, easily perform a \keyword{return-to-libc} attack, because the address of each library function would be the same on every run.
...
\item sudo.
  Sudo is not an OpenBSD invention, but its long-time maintainer is an OpenBSD developer, and it runs everywhere! (OpenBSD itself now ships a simpler replacement, \keyword{doas}.)
...
Containers are not virtual machines: instead of emulating motherboard peripherals, they share the host operating system's kernel and use kernel features (such as namespaces and control groups) to isolate programs from each other. They are much lighter-weight than virtual machines, but because they share a kernel with the host, they provide a weaker isolation boundary.
...
\item Identity Verification.
  ...
  DNS answers are usually correct, but plain DNS gives the client no way to verify them.
  A spoofed answer is accepted just as readily as a real one, and the program happily connects to whatever address it was handed.
