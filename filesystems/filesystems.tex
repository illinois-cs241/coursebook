...
\begin{enumerate}
\item Empircally, they both produce numbers that look random enough.
...
\item Man page authors pose a hypothetical attack where an attacker exhausts the entropy pool and guesses the seeding bits, but that attack has yet to be implemented.
...
In order to make it seem like all the files are on an SSD, the StoreMI matches the pattern of file access.
