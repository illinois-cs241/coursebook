...
The third call `lstat' we will discuss when we introduce symbolic links. In addition to access, creation, and modified times, the stat structure includes the inode number, length of the file and owner information.
...
Ans: Use \keyword{S\_ISDIR} to check the mode bits stored in the stat structure

And to check if a file is regular file use \keyword{S\_ISREG},
...
There is a system call, \keyword{int chmod(const char *path, mode\_t mode);} but we will concentrate on the shell command. There's two common ways to use \keyword{chmod} ; with an octal value or with a symbolic string:
...
Use the \keyword{S\_ISDIR} macro to check the mode bits in the stat struct:
