...
The program would just use something like \keyword{open} or \keyword{mkdir}.
...
The other is programs need to explicity exclude subdirectories from a search, otherwise the search may take a very long time.
...
If not, then the kernel will most likely just mark an error.
...
\item Distributed. Sometimes, the filesystem should be single machine fault tolerant. Hadoop and other distributed file system allow you to do just that.
\end{itemize}
