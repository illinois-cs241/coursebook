...
Note that \keyword{...} is NOT a valid representation of any directory.
This not the grandparent directory.
It \emph{could} however be the name of a file on disk.
Though confusingly, \keyword{zsh} provides this as a handy shortcut to the grandparent directory should it exist.
...
While interacting with a file in C is done by using \keyword{open} to open the file and then \keyword{read} or \keyword{write} to interact with the file before calling \keyword{close} to release resources, directories have special calls such as, \keyword{opendir}, \keyword{closedir} and \keyword{readdir}.
...
After a call to \keyword{fork()}, either (XOR), the parent or the child can use \keyword{readdir()}, \keyword{rewinddir()} or \keyword{seekdir()}.
