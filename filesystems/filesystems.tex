...
\begin{enumerate}
\item Empircally, they produce numbers that look random enough.
\item \keyword{/dev/random} May block at an inconvenient time. If one is programming a service for high scalability and relies on \keyword{/dev/random} the issue becomes an attacker can reliably exhaust the entropy pool and cause the service to block.
\item Man page authors pose a hypothetical attack where an attacker exhausts the entropy pool and guesses the bits, but that attack has yet to be implemented.
\item Some operating system don't have a true \keyword{/dev/random} like MacOS.
\item Security experts will talk about Computational Security vs Information Theoretic security, more on this article \href{https://www.2uo.de/myths-about-urandom}{Urandom Myths}
\end{enumerate}
