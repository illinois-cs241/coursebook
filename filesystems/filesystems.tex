...
You may have encountered the old unix adage, "everything is a file". In most UNIX systems, files operations provide an interface to abstract many different operations. Network sockets, hardware devices and even data on disk are all represented by a file-like object.
...
Although filesystems are usually thought of as a kind of tree, most filesystems are usually a directed graph, a model we will explore in depth later in this chapter.

\subsection{Examples of filesystems}
...
\begin{enumerate}
  \item \keyword{disk block}
  \item \keyword{superblock}
  \item \keyword{inode}
\end {enumerate}
...
\subsection{How does an inode store file contents?}\label{how-does-an-inode-store-file-contents}
...
\subsection{Directory conventions}
...
Note that \keyword{...} is NOT a valid representation of any directory (this not the grandparent directory). It \emph{could} however be the name of a file on disk.
...
While this is dependant on the underlying filesystem, the POSIX standard says no you may not! The \keyword{ln} command will only allow root to do this and only if you provide the \keyword{-d} option. However even root may not be able to perform this because most filesystems prevent it!
...
Not all virtual filesystems are as abstract as the ones above. Some, like \keyword{OverlayFS} are a good example of how a virtual filesystem may simply augment the functionality of what a filesystem provides. OverlayFS takes in a path to a directory containing some data (a lower directory), and a directory that is preferably empty (a work directory) when it is mounted. It then pretends to hold the contents of the lower directory and will allow a user to read or modify the contents of those files without checking permissions. While the user may think that they are editing file that they normally do not have access to, OverlayFS is not actually writing the changes back to the lower directory, but instead to a file in the work directory, thus writing to an `overlay' on top of a real filesystem (if you are familiar with photoshop or other image editing tools, it is similar to the idea of a layer).

This filesystem is used to implement containers (such as Docker) and other related isolation tools. It is commonly used in conjunction with \keyword{chroot}.

\section{Managing files and filesystems}
...
\subsection{Managing Filesystems}\label{how-do-i-find-out-what-filesystems-are-currently-available-mounted}
...
The \keyword{PROT\_READ | PROT\_WRITE} options specify the virtual memory protection. The option \keyword{PROT\_EXEC} (not used here) can be set to allow CPU execution of instructions in memory (e.g.~this would be useful if you mapped an executable or library).
