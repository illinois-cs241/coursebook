...
The price is extra disk reads.
Once the inode is in memory, finding a data block through a direct pointer takes one read, through a single indirect block two, a double indirect block three, and a triple indirect block four.
In practice the filesystem caches indirect blocks, so for sequential reads that cost is shared by the 1024 data blocks each indirect block describes.
...
Each octal digit is three bits: the most significant bit (4) corresponds to read privileges, the middle bit (2) to write privileges and the least significant bit (1) to execute privileges.
...
This owner's user ID (\keyword{uid}) can be found inside the \keyword{st\_uid} field of a \keyword{struct stat} with a call to \keyword{stat}.
Similarly, the group ID (\keyword{gid}) is set as well, and stored in \keyword{st\_gid}.
...
This is where you stripe data with RAID-0 across several RAID-1 mirrored pairs -- with four disks, that's two mirrored pairs.
You get the speedup of striping, reads can be served by either disk of a pair, and, as with RAID-1, you pay for twice the raw storage.
Any single disk can fail, and its data can be rebuilt from its mirror.
The array can even survive several failures, as long as no two failed disks are in the same mirrored pair; losing both disks of one pair loses data.
With four disks, once one disk has failed, a second random failure hits its partner with probability $1/3$.
...
One disadvantage of a dedicated parity disk is that every write -- no matter which data disk it goes to -- also has to update the parity disk.
The parity disk ends up doing as many writes as all the data disks combined, so it becomes the bottleneck for writes.
Strictly speaking, this write-bottleneck argument is the standard criticism of RAID-4, which stripes whole blocks across the data disks and keeps all the parity on one disk.
RAID-3 stripes at the byte level, so every read or write touches every disk anyway.
RAID-5, below, fixes the bottleneck by spreading the parity across all the disks.
...
RAID-5 is similar to RAID-4 except that the check block (parity information) is assigned to different disks for different blocks.
The check-block is `rotated' through the disk array.
RAID-5 provides better write performance than RAID-4 (and RAID-3) because there is no longer the bottleneck of the single parity disk, and reads can be spread over all the disks.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.8\textwidth,alt={An inode whose two direct entries point to data blocks 7 and 3 and whose indirect entry points to block 2, which in turn lists blocks 5 and 4. Blocks 7, 3 and 5 are full and block 4 is partly filled.}]{filesystems/images/sample_file.png}
...
The file presented above is interesting since it uses all of its direct blocks, one full data block referenced by its indirect block, and partially uses a second data block referenced by the indirect block.
...
First we'll focus on files and assume that we are writing a byte at offset 6KiB into our file.
To perform a write on a file at a particular offset, the filesystem first works out which of the file's blocks holds that offset: $6\text{KiB} / 4\text{KiB} = 1$ with a remainder of $2$KiB, so we want block index 1 of the file (its 2nd block), 2KiB in.
Block index 1 is covered by the inode's second direct entry, Direct \#1, which says data block $3$.
So we go to the start of the data blocks, find data block $3$, and perform our write 2KiB into that block -- the first four kibibytes of the file live in block 7, which we skipped.
