...
If you want to read the entire file, keep reading direct blocks until you've read several bytes equal to the size of the file.
...
Write access to a directory that will allow you to create or delete new files or directories inside.
...
We will base it off of the \keyword{minixfs}, a very simple filesystem that happens to be the first filesystem that Linux ran on.
...
\item Caching. Linux does a very good job of caching file system operations like finding inodes. This makes disk operations seem nearly instant. If you want to see a slow system, look at windows with FAT/NTFS. Disk operations need to be cached by the application, or it will burn through the CPU.
...
\item Distributed. Sometimes, you want your filesystem to run on a fault tolerant scale across multiple computers. Hadoop and other distributed file system allow you to do just that.
\end{itemize}
...
We aren't trying to sell you AMD chipsets, but the featureset of StoreMI warrants a mention.
