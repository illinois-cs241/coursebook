...
\section{Examples of filesystems}
...
% TODO connect this sections better. Maybe talk about ownership first?
...
\keyword{chmod(const char *path, mode\_t mode);}
...
\subsection{How does the operating system load my process and libraries into memory?}

By mapping the files' contents into the address-space of the process. If many programs only need read-access to the same file (e.g. \keyword{/bin/bash}, the C library) then the same physical memory can be shared between multiple processes.
...
Note for simple sequential processing memory mapped files are not necessarily faster than standard `stream-based' approaches of \keyword{read} / \keyword{fscanf} etc.
...
Easy - Use \keyword{mmap} without a file - just specify the \keyword{MAP\_ANONYMOUS} and \keyword{MAP\_SHARED} options!
