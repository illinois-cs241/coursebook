...
Writing to the filesystem or FS involved writing to a magnetic tape and reading from that tape \todo{citation needed}.
...
A filesystem, as we'll define more concretely later, is anything that satisfies the API of a filesystem, it may not be a hard disk.
...
\textbf{The last piece of background is an important one.} In this chapter, we will refer to sizes of files in the ISO-compliant KiB or Kibibyte. The *iB family is short for power of two storage.
...
We will do this in the book and in the networking chapter to keep consistency and not to confuse anyone.
...
\begin{enumerate}
  ...
  \item It must support common filesystem operations, such as \keyword{open}, \keyword{read}, \keyword{write}. At a minimum, it needs to be opened and closed.
\end{enumerate}
...
Meaning that \keyword{/} is resolved by the \keyword{ext4} filesystem in our case, but \keyword{/proc/2} is resolved by the \keyword{procfs} system even though it contains \keyword{/} as a subsystem.
...
Many many filesystems aren't \keyword{fseek}able meaning that you can't move to an arbitrary point in the file.
...
Four bytes pointers are needed because we want to address 32 bits worth of blocks.
...
Unituitively, \keyword{...} does not refer to the `grandparent' directory (parent of the parent). More generally, there are only reserved aliases for the current directory and the parent directory.
However, \keyword{...} \emph{could} however be the name of a file or direcotyr on disk. (You can try this with \texttt{mkdir ...}).
Confusingly, the shell \keyword{zsh} does interprent \keyword{...} as a handy shortcut to the grandparent directory (should it exist) while expanding shell commands.

Additional facts about name-related conventions.

\begin{enumerate}
\item Files that start with '.' on disk are conventionally considered 'hidden' and will not be listed by programs like \keyword{ls} without additional flags (\keyword{-a}).
...
While modeling the filesystem as a tree would imply that every inode has a unique parent directory, links allow inodes to present themselves as files in multiple places, potentially with different names, thus leading to an inode having multiple parents directories.
...
The C library provides a function \keyword{realpath} to compress the path or get the realpath.
...
It is worth noting that the \keyword{rwx} bits for a directory have slightly different meaning.
...
Google reports 2-10\% of disks fail per year Now multiply that by 60,000+ disks in a single warehouse must survive failure of not just a disk, but a rack of servers or a whole data center
...
a good choice. After the superblock, we'll keep a map of which inodes are being used. The nth bit is set if the nth
inode -- $0$ being the inode root -- is being used. Similarly, we store a map recording which data blocks are used.
Finally, we have an array of inodes followed by the rest of the disk - implicitly partitioned into data blocks. It's
important to note that there may not be any real distinction from one data block to the next from the perspective of the
hardware components of the disk. Thinking about the disk as an array of data blocks is simply something we do so that we
...
drawn arrows mapping data block numbers in the inode to their locations on disk, these aren't pointers so much as
...
typically stored in the inode. This is because the filesystem isn't aware of the actual contents of what is in a file.  That data is considered the user's. However, we can compute an upper bound by looking at how many blocks the file uses.
...
calculations, we did not take this block into account while computing the file size. This would instead be counted as the
...
Note that when a user writes the first byte, a data block will be allocated. Since each data block is $4KiB$, we find that $4KiB$ is the minimum disk usage for a non-empty file.
...
Finding maximum is slightly more involved. As we saw earlier in this chapter, a filesystem with this structure can have $1024$ data block numbers in one
indirect block. This implies that the maximum filesize can be $2*4KiB + 1024*4KiB = 4MiB + 8KiB$ (after accounting for the direct blocks as well).
However, on disk we also store the indirect block itself. This means that an additional $4KiB$ of overhead will be used to account for the indirect block, so the total
disk usage will be $4MiB + 12KiB$.
...
While it would seem like we always want this idea scenario, it puts a very restrictive limit on the maximum filesize.
Attempting to remedy this by increasing the number of direct blocks seems promising, but note that this requires increasing the size of an inode and reducing the amount of space availible to store user data
-- a tradeoff you will have to evaluate for yourself.
...
direct data block number. In our case it is \#7. Then we find the 7th data block from the \textit{start} of all data blocks.
...
our data continues on the 5th data block from the beginning. We do the same for data block \#4 and we stop after because
